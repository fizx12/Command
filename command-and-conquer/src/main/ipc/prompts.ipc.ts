...
export function registerPromptHandlers(promptCompilerService: PromptCompilerService, fileStore: FileStore): void {
  ipcMain.handle('prompts:compile', async (_event, projectId: string, taskId: string, step: string) => {
    try {
      const promptStep: PromptStep = step === 'implement' ? 'implement' : 'plan';
      const result = await promptCompilerService.compile(projectId, taskId, promptStep, 'MAX')
  ...
  ipcMain.handle('prompts:preview', async (_event, projectId: string, taskId: string, step: string) => {
    try {
      const promptStep: PromptStep = step === 'implement' ? 'implement' : 'plan';
      const result = await promptCompilerService.preview(projectId, taskId, promptStep, 'MAX')
      return { error: false, data: result }
    } catch (error) {
      return {
        error: true,
  ...
  ipcMain.handle('prompts:build-architect', async (_event, projectId: string, taskId: string) => {
    try {
      const payload = await promptCompilerService.buildArchitectPayload(projectId, taskId);
