...
export class ProjectService {
  ...
  async listProjects(): Promise<Project[]> {
    const scanPath = 'workspace/projects';
    console.log('[ProjectService] Scanning for projects in:', this.fileStore.resolvePath(scanPath));
    const projectDirs = await this.fileStore.listDirectories(scanPath);
    const projects: Project[] = [];
    
    for (const dirName of projectDirs) {
      try {
        const project = await this.fileStore.readJSON<Project>(`workspace/projects/${dirName}/overview/project.config.json`);
        if (project) {
  ...
  async deleteProject(id: string): Promise<void> {
    await this.fileStore.remove(`workspace/projects/${id}`);
  }
    ...
    try {
      ...
      for (const fileName of files) {
        if (fileName.endsWith('.json')) {
          try {
            const repo = await this.fileStore.readJSON<Repository>(`workspace/projects/${projectId}/overview/repos/${fileName}`);
            if (repo) {
              repos.push(repo);
            }
          } catch {
